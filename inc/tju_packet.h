...
/*
 * 课程自定义的 20-byte TJU 线格式。
 * 字段通过 tju_packet.c 的固定偏移逐个编码，不能直接 memcpy 此 C 结构体：
 * 编译器对齐会使 sizeof(tju_header_t) 在常见平台上大于 20。
 * 当前格式没有 checksum 字段，tju_validate_packet 只做结构/长度校验。
 */
...
// 便于在内存中表达字段的结构；它不是可直接发送的 wire image。
typedef struct {
    ...
    uint8_t ext;				//1 byte  额外的数据 在这个项目里是为了将header的大小凑整到20bytes 没有实际意义
} tju_header_t;

// 临时报文对象：data 由对象拥有，free_packet 负责释放。
typedef struct {
	tju_header_t header;
...
/*
 ...
 根据各个字段的偏移量
 找到并返回对应的字段。调用者必须先保证缓冲区至少有 20 bytes。
*/ 
